...
Skip intro and Escape finish immediately. Reduced motion shows the complete descriptions and waits for “Enter my space”, then enters without an orbit. Resizing preserves the current arrival phase. Navigation stays locked until completion, then focus returns to About me. The sequence runs on each page load. Pen drawing remains the default; an existing saved theme preference is retained.
