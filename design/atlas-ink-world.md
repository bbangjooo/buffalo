# 펜드로잉·Low-poly 테마 — 디자인 명세 v1.1

| 항목 | 발행 기준 |
| --- | --- |
| 문서 ID | `ATLAS-DESIGN-001` |
| 버전 / 발행일 | 1.1 / 2026-09-13 |
| 상태 | 발행 · 구현 반영 |
| 기준 구현 | [`a8d4c65`](https://github.com/bbangjooo/buffalo/commit/a8d4c650109f9a508ef7ba7f25dc6036f73e2259) |
...
아래 수량과 좌표는 v1.1의 기준값이다. 후속 디자인에서는 구성과 수량을 바꿀 수 있으며,
...
- 데스크톱에서는 Explore 아래, 작은 화면에서는 Explore 옆에 배치한다. 트리거 너비는 124px로 고정해 선택명이 바뀌어도 줄바꿈과 위치가 변하지 않는다.
- Explore에서는 왼쪽 복귀·위치 안내 아래, 연주 좌석에서는 Stand up 아래에 둔다. 읽기·리듬게임 화면의 기존 전용 조작부를 유지한다.
- 로딩·방 이동·아트 테마 전환 중에는 선택을 막는다. 이미 선택된 항목은 다시 적용하지 않는다.
- 방향키·Home/End로 선택지를 이동하고 Escape·바깥 클릭·메뉴 밖 포커스로 닫는다.
- 320px 좌석 화면의 로딩 알림은 기본 상단 198px로 배치하여 키보드 안내문과 간격을 확보한다.
