...
export default class ClassicMeadow {
  ...
  constructor(private application: Application, model: THREE.Group) {
    this.root.name = 'ClassicLowPolyMeadow';
  ...
  setEnabled(enabled: boolean) { this.enabled=enabled; this.root.visible=enabled; if(enabled)this.applyAtmosphere(); }
  ...
  dispose() {
    ...
    this.tiles.forEach(tile => { tile.grass.dispose(); tile.flowers.dispose(); tile.rocks.dispose(); });
