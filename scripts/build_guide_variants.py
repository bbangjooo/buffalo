"""Author separate faceted and smooth guides without overwriting the v2 guide.

/Applications/Blender.app/Contents/MacOS/Blender --background --factory-startup \
    --python scripts/build_guide_variants.py -- faceted
# Repeat with -- smooth for the second style.
...
VARIANT = sys.argv[sys.argv.index('--')+1] if '--' in sys.argv else 'faceted'
if VARIANT not in {'faceted','smooth'}:
    raise ValueError('Choose faceted or smooth after --')
SMOOTH = VARIANT == 'smooth'
ASSET_NAME = 'character-guide-' + VARIANT
CONFIG = ({'segments':64,'headLevels':48,'bodyLevels':44,'ovalSegments':40,'ovalRings':24,'markSegments':48,'curveSteps':12}
          if SMOOTH else
          {'segments':12,'headLevels':9,'bodyLevels':8,'ovalSegments':8,'ovalRings':6,'markSegments':12,'curveSteps':4})
...
def oval(name, center, radii, mat, target=root, segments=None, rings=None, tilt=0):
    segments=segments or CONFIG['ovalSegments'];rings=rings or CONFIG['ovalRings']
    points=[]
...
def contour(name,profile,mat,target,segments=None,levels=None):
    segments=segments or CONFIG['segments'];levels=levels or CONFIG['headLevels']
    points=[];faces=[]
    ...
    for tri in triangles:
        ...
        if (b-a).cross(c-a).z > 1e-9:
            front.append((a,b,c))
            front_normals.append(tuple(Vector((obj.data.vertices[i].normal.x,
                obj.data.vertices[i].normal.z,-obj.data.vertices[i].normal.y)) for i in tri))
    SURFACES[id(profile)]={'triangles':front,'normals':front_normals,
...
contour('PlumpCreamBody',BODY,CREAM,root,levels=CONFIG['bodyLevels'])
# Raised, short rounded ears. The tips turn outward like the reference plush.
for sign in (-1,1):
    oval('StrawberryEar'+str(sign),(sign*.365,1.375,-.012),(.084,.108,.080),RED,head,tilt=-sign*.42)
...
def patch(name,outline,center,profile,mat,target=head,offset=.008,rings=5):
    ...
    points=[];faces=[];indices={};normals=[]
    for (a,b,c),(na,nb,nc) in zip(SURFACES[id(profile)]['triangles'],SURFACES[id(profile)]['normals']):
        ...
        for piece in pieces:
            ...
            for x,y in polygon:
                ...
                if key not in indices:
                    indices[key]=len(points);points.append((x,y,z+offset))
                    # Barycentric interpolation reproduces the normal field of
                    # the dense skin. Geometry alone is insufficient: averaging
                    # only a small eye/cheek patch's normals creates a shading
                    # seam at its edge even when it fits the body exactly.
                    ...
                    skin_normal=(na*wa+nb*wb+nc*(1-wa-wb)).normalized()
                    normals.append(v(skin_normal))
    ...
    for face in faces:
        sample=sum((Vector(points[index]) for index in face),Vector())/len(face)
        expected=surface(profile,sample.x,sample.y)+offset
        error=abs(sample.z-expected)
        assert error<3e-5,(name,'marking must lie on the actual skin plane',error)
        PATCH_FIT_ERRORS.append(error)
    obj=mesh(name,points,faces,mat,target)
    if SMOOTH:
        obj.data.normals_split_custom_set_from_vertices(normals)
        obj.data.update()
        obj['skinNormalTransfer']=True
    return obj

def ellipse(cx,cy,rx,ry,n=None):
    n=n or CONFIG['markSegments']
...
def bezier(points,steps=None):
    steps=steps or CONFIG['curveSteps']
    out=[]
...
for sign,pivot in [(-1,arm),(1,arm_left)]:
    oval('CreamRaisedForearm'+str(sign),(sign*.298,.618,.355),(.103,.137,.123),CREAM,pivot,tilt=-sign*.22)
    oval('RedRoundedPaw'+str(sign),(sign*.277,.720,.444),(.096,.088,.091),RED,pivot,tilt=-sign*.22)
for sign,pivot in [(-1,foot_right),(1,foot_left)]:
    oval('RedFlatFoot'+str(sign),(sign*.245,.073,.13),(.184,.073,.225),RED,pivot,)
oval('RoundCreamTail',(0,.235,-.428),(.151,.154,.145),CREAM,root,)
...
assert (10000<triangles<100000) if SMOOTH else (900<triangles<2400)
...
bpy.ops.wm.save_as_mainfile(filepath=str(ROOT/'assets'/(ASSET_NAME+'.blend')))
bpy.ops.object.select_all(action='DESELECT')
for obj in [root,*root.children_recursive]:obj.select_set(True)
bpy.ops.export_scene.gltf(filepath=str(ROOT/'public/Room'/(ASSET_NAME+'.glb')),export_format='GLB',use_selection=True,
    export_apply=True,export_cameras=False,export_lights=False,export_animations=False,export_extras=True,
    export_draco_mesh_compression_enable=True,export_draco_mesh_compression_level=6)
manifest={'style':VARIANT,'root':'GuideCharacter','facing':'+Z','height':round(maximum[2],6),'width':round(maximum[0]-minimum[0],6),
    'depth':round(maximum[1]-minimum[1],6),'footY':round(minimum[2],6),'triangles':triangles,'meshes':len(meshes),
    'bytes':(ROOT/'public/Room'/(ASSET_NAME+'.glb')).stat().st_size,
    ...
    'skinTopology':{'headSegments':CONFIG['segments'],'headLevels':CONFIG['headLevels'],'bodySegments':CONFIG['segments'],'bodyLevels':CONFIG['bodyLevels']},
...
manifest['source']='assets/'+ASSET_NAME+'.blend'
manifest['export']='public/Room/'+ASSET_NAME+'.glb'
manifest['smoothFaces']=sum(poly.use_smooth for obj in meshes for poly in obj.data.polygons)
manifest['flatFaces']=sum(not poly.use_smooth for obj in meshes for poly in obj.data.polygons)
manifest['normalTreatment']='Interpolated skin vertex normals on all fitted markings' if SMOOTH else 'Flat face normals on every surface'
manifest['maxPatchSurfaceError']=max(PATCH_FIT_ERRORS,default=0)
catalog_file=ROOT/'assets/character-guide-variants.json'
catalog=json.loads(catalog_file.read_text()) if catalog_file.exists() else {'baseline':'character-guide.glb','variants':{}}
catalog['variants'][VARIANT]=manifest
...
for name,location in [('front',(0,.93,4)),('side',(4,1.05,0)),('three-quarter',(2.6,1.55,4)),('back',(0,1.1,-4))]:
    ...
    scene.render.filepath=str(ROOT/'assets'/(ASSET_NAME+'-'+name+'.png'))
